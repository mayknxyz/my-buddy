...
The slug (date) of the entry to edit, optionally followed by field=value pairs. Defaults to today's date if empty.
...
Read `src/content/journals/{slug}.md`. If not found, try fuzzy matching.
...
```text
{PERSONA NAME} | Updated journal: src/content/journals/{slug}.md
