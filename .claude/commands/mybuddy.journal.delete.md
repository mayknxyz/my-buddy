...
The slug (date) of the entry to delete.

## Procedure

### 1. Resolve Entry

If `$ARGUMENTS` is empty, list recent entries in `src/content/journals/` and ask the user to pick one.

Read `src/content/journals/{slug}.md`. If not found, inform the user.
...
If confirmed, delete the file at `src/content/journals/{slug}.md`.

### 4. Output

```
{PERSONA NAME} | Deleted journal: src/content/journals/{slug}.md
```
